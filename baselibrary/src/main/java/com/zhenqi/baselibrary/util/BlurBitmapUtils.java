...
public class BlurBitmapUtils {
    ...
    public static Bitmap getCompleteBlurBitmap(Context context, Bitmap bitmap) {
        return getCompleteBlurBitmap(context, bitmap, BLUR_RADIUS);
    }

    public static Bitmap getEightBlurBitmap(Context context, Bitmap bitmap) {
        return getEightBlurBitmap(context, bitmap, BLUR_RADIUS);
    }
    ...
    public static Bitmap getEightBlurBitmap(Context context, Bitmap bitmap, int radius) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            // 将缩小后的图片做为预渲染的图片。
            float width = bitmap.getWidth();
            float height = bitmap.getHeight();
            Bitmap inputBitmap = Bitmap.createScaledBitmap(bitmap, (int) (width * 0.4f), (int) (height * 0.4f), false);
            // 创建一张渲染后的输出图片。
            Bitmap outputBitmap = Bitmap.createBitmap(inputBitmap);
            // 创建RenderScript内核对象
            RenderScript rs = RenderScript.create(context);
    ...
    public static Bitmap getCompleteBlurBitmap(Context context, Bitmap bitmap, int radius) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            // 将缩小后的图片做为预渲染的图片。
//            Bitmap inputBitmap = Bitmap.createScaledBitmap(bitmap, SCALED_WIDTH, SCALED_HEIGHT, false);
            // 创建一张渲染后的输出图片。
